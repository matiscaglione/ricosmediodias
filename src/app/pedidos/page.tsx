...
export default function HistorialPedidosPage() {
  ...
  // ESCUCHA REALTIME CON RETARDO PARA CARGAR DETALLES COMPLETOS
  useEffect(() => {
    cargarPedidosRango();
    ...
    const canal = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          // Pequeña espera para asegurar que la inserción de detalle_pedidos haya terminado en Supabase
    ...
    const filas = pedidosFiltrados.map((p) => {
      ...
      const detalleStr = (p.detalle_pedidos || [])
        .map((i) => {
          ...
          if (i.salsas?.nombre) str += ` (Salsa: ${i.salsas.nombre})`;
  ...
  const pedidosFiltrados = pedidos.filter((p) => {
    ...
    const itemsMatch = (p.detalle_pedidos || []).some((item) => {
      ...
      const salsaNom = (item.salsas?.nombre || '').toLowerCase();
                    ...
                    {pedido.detalle_pedidos?.map((item) => (
                      <div key={item.id} className="flex justify-between text-sm">
                        <div className="flex-1">
                          ...
                          {item.salsas?.nombre && (
                            <span className="text-xs font-black text-red-800 block pl-3">
                              🍝 {item.salsas.nombre}
                            </span>
                          )}
