...
export default function ReportesPage() {
  ...
  const totalEnviosCant = pedidos.filter(
    (p) => p.tipo_entrega === "ENVIO",
  ).length;
  const totalRetirosCant = pedidos.filter(
    (p) => p.tipo_entrega === "RETIRO",
  ).length;
  ...
  pedidos.forEach((p) => {
    p.detalle_pedidos?.forEach((d) => {
      ...
      resumenPlatos[nombrePlato] =
        (resumenPlatos[nombrePlato] || 0) + d.cantidad;

      if (d.guarniciones?.nombre) {
        const nombreGuarni = d.guarniciones.nombre;
        resumenGuarniciones[nombreGuarni] =
          (resumenGuarniciones[nombreGuarni] || 0) + d.cantidad;
      }
    });
  });
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm font-bold text-gray-700">
            Resumen de ventas e historial financiero
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link
            href="/"
            className="bg-blue-600 text-white text-sm px-3 py-2 rounded font-extrabold hover:bg-blue-700"
          >
            ➕ Tomar Pedido
          </Link>
          <Link
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white p-5 rounded-lg shadow-sm border-2 border-gray-300">
          ...
          <span className="text-xl font-black" style={styleTextoNegro}>
            {formatearMoneda(totalPlatosMonto)}
          </span>
        </div>
        ...
        <div className="bg-white p-5 rounded-lg shadow-sm border-2 border-gray-300">
          ...
          <div
            className="text-xs font-bold mt-1 space-y-0.5"
            style={styleTextoNegro}
          >
            <div>
          ...
          {Object.keys(resumenPlatos).length === 0 ? (
            ...
            <div className="space-y-2">
              {Object.entries(resumenPlatos).map(([plato, cantidad]) => (
                <div
                  key={plato}
                  className="p-2.5 bg-gray-50 rounded border border-gray-300 flex justify-between items-center"
                >
