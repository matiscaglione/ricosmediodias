...
export default function ReportesPage() {
  ...
  // --- CÁLCULO DE TOTALES Y BEBIDAS VENDIDAS ---
  ...
  pedidos.forEach((p) => {
    p.detalle_pedidos?.forEach((d) => {
      if (d.bebidas && d.bebidas.nombre) {
        ...
        rankingBebidasMap[nombreBebida] = (rankingBebidasMap[nombreBebida] || 0) + cant;
        totalUnidadesBebidas += cant;
      }
    });
  });

  const resumenBebidas = Object.entries(rankingBebidasMap)
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto font-sans bg-gray-100 min-h-screen space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-300 text-center">
          <p className="text-xs font-bold text-gray-500 uppercase">🥤 Bebidas Vendidas</p>
          <p className="text-2xl font-black text-blue-900">{totalUnidadesBebidas} u.</p>
        </div>
      </div>

      {/* SECCIÓN RESUMEN DE BEBIDAS EN EL CIERRE */}
      {resumenBebidas.length > 0 && (
        <div className="bg-white p-5 rounded-lg border border-gray-300 space-y-3">
          <h2 className="text-md font-black text-blue-900 border-b pb-2">
            🥤 Desglose de Bebidas Vendidas en el Período
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {resumenBebidas.map((b, idx) => (
              <div key={idx} className="flex justify-between items-center bg-blue-50 p-2 rounded border border-blue-200 text-sm">
                <span className="font-bold text-gray-800">{b.nombre}</span>
                <span className="font-black bg-blue-600 text-white px-2 py-0.5 rounded text-xs">
                  {b.cantidad} u.
                </span>
              </div>
            ))}
                  ...
                  <div className="text-xs space-y-1 pl-2 border-l-2 border-gray-400">
                    {p.detalle_pedidos?.map((d, idx) => (
                      <div key={idx} className="font-bold text-gray-800">
                        {d.bebidas ? (
                          <span className="text-blue-900">• 🥤 {d.cantidad}x {d.bebidas.nombre}</span>
                        ) : (
                          <span>• {d.cantidad}x {d.menus?.nombre || 'Plato'} {d.guarniciones ? `(+ ${d.guarniciones.nombre})` : ''}</span>
                        )}
                      </div>
                    ))}
                    {p.observaciones && (
